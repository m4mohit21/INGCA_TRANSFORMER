...
def count_psnr(dataset_path,testing_dataset,epoch,
               valid_data='2013',
               setting='',
               flipped=False,
               thresh=0.5):
    ...
    gt_folder = f'{dataset_path}/{testing_dataset}/gt_imgs'
...
def count_PRF(dataset_path,epoch,
               valid_data='2016',
               setting='',
               flipped=False,
               thresh=0.5):
    ...
    print(valid_data)
    folder = './vis/epoch' + str(epoch)
    gt_folder = f'{dataset_path}/{valid_data}/gt_imgs'
    ...
    for im in gt_imgs:
        ...
        # Ensure pred_image has the same shape as gt_image
        if pred_image.shape != gt_image.shape:
            print(f"Resizing predicted image {im} from shape {pred_image.shape} to match ground truth shape {gt_image.shape}")
        ...
        precision_value = precision_score(gt_image.flatten(), pred_image.flatten(), pos_label=0,average='binary')
        recall_value = recall_score(gt_image.flatten(), pred_image.flatten(),pos_label=0, average='binary')
        f1score_value = f1_score(gt_image.flatten(), pred_image.flatten(),pos_label=0, average='binary')

        # Append individual scores to the respective lists
        precision_list.append(precision_value)
        recall_list.append(recall_value)
        f1score_list.append(f1score_value)

        # Print the metrics for the current image
        print(f"Image {im}: Precision: {precision_value:.4f}, Recall: {recall_value:.4f}, F1-Score: {f1score_value:.4f}")
    ...
    return precision_list, recall_list, f1score_list, avg_precision, avg_recall, avg_f1score